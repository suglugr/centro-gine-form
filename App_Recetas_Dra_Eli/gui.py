...
# --- PDF CONFIG ---
TOP_MARGIN_PTS   = 4 * 28.3465          # 4 cm en puntos
FONT_SIZE        = 9

# Hoja pestaña 1: 21 x 14 cm
PAGE_WIDTH       = 21   * cm
PAGE_HEIGHT      = 14   * cm
CUSTOM_PAGE_SIZE = (PAGE_WIDTH, PAGE_HEIGHT)

# Hoja pestaña 2 (imágenes): carta 21.59 x 27.94 cm
...
class MedicalApp:
    ...
    def _build_tab_form(self):
        ...
        ctk.CTkLabel(self.tab_form, text="Medicamentos e Indicaciones").pack(anchor="w", padx=20)
        self.txt_meds = ctk.CTkTextbox(self.tab_form, height=300)
        self.txt_meds.pack(fill="x", padx=20, pady=5)

        ctk.CTkLabel(self.tab_form, text="Notas Privadas (Solo Uso Interno)").pack(anchor="w", padx=20)
        ...
        ctk.CTkButton(btn_row, text="Guardar Paciente",
                      fg_color="#27ae60", command=self.save_patient).pack(side="left", padx=10)
        ctk.CTkButton(btn_row, text="Imprimir PDF",
                      fg_color="#2980b9", command=self.generate_patient_pdf).pack(side="left", padx=10)
        ctk.CTkButton(btn_row, text="Limpiar Formulario",
                      fg_color="#c0392b", command=self.clear_form).pack(side="left", padx=10)
    ...
    def _build_tab_images(self):
        ...
        for i in range(9):
            ...
            ctk.CTkButton(b_frame, text="Agregar", width=55,
                          command=lambda idx=i: self.add_image(idx)).pack(side="left", padx=2, pady=2)
            ctk.CTkButton(b_frame, text="Eliminar", width=55, fg_color="#c0392b",
                          command=lambda idx=i: self.remove_image(idx)).pack(side="right", padx=2, pady=2)
        ...
        ctk.CTkLabel(self.tab_images,
                     text="Análisis de Imagen / Observaciones").pack(anchor="w", padx=20)
    ...
    def _build_tab_records(self):
        ...
        self.ent_search = ctk.CTkEntry(top_bar,
                                       placeholder_text="Buscar por nombre...", width=300)
        ...
        ctk.CTkButton(top_bar, text="Exportar Datos",
                      fg_color="#8e44ad", command=self.export_data).pack(side="right", padx=5)
        ctk.CTkButton(top_bar, text="Importar Datos",
                      fg_color="#d35400", command=self.import_data).pack(side="right", padx=5)
        ctk.CTkButton(top_bar, text="Reparar BD",
                      fg_color="#e67e22", command=self.repair_database).pack(side="right", padx=5)
    ...
    def load_selected(self):
        ...
        if row:
            self.clear_form()
            self.current_patient_id, name, age, date, meds, notes, imgs, img_desc = row
            ...
            self.lbl_img_patient.configure(text=f"Paciente Actual: {name} ({age})")
    ...
    def export_data(self):
        ...
        messagebox.showinfo("Exportar", "Datos e imágenes exportados correctamente.")
    ...
    def repair_database(self):
        with sqlite3.connect(DB_NAME) as conn:
            ...
            for r_id, img_json in rows:
                ...
                fixed = [
                    p if p and os.path.exists(os.path.join(MEDIA_FOLDER, p))
                    else None
                    for p in paths
                ]
    ...
    def _draw_header(self, c, w, h, margin_l, margin_r, top_margin):
        """
        Cabecera común: nombre (izq) | edad (centro-der) | fecha (der).
        Devuelve la Y lista para el contenido.
        """
        y = h - top_margin
        ...
        if age:
            c.drawString(w * 0.65, y, age)
        if date:
            c.drawRightString(w - margin_r, y, date)

        return y - 18   # separador cabecera → cuerpo

    # ── PDF PESTAÑA 1: medicamentos (21 x 14 cm) ──────────────────────────────
    def generate_patient_pdf(self):
        ...
        y = self._draw_header(c, w, h, ml, mr, TOP_MARGIN_PTS)
    ...
    # ── PDF PESTAÑA 2: imágenes (carta 21.59 x 27.94 cm) ─────────────────────
    def generate_image_pdf(self):
        """
        Hoja carta (21.59 x 27.94 cm).
        Layout:
          - Margen superior 4 cm
          - Línea de cabecera: nombre | edad | fecha
          - Título "Estudios" centrado y en negrita
          - Texto de observaciones
          - Grilla de imágenes (3 columnas)
        """
        name     = self.ent_name.get()
        age      = self.ent_age.get()
        date     = self.ent_date.get()
        ...
        # ── Grilla de imágenes (3 columnas) ──
        # Calculamos ancho disponible y repartimos en 3 columnas
        usable_w = w - ml - mr
        h_gap    = 0.4  * cm
        v_gap    = 0.4  * cm
        img_w    = (usable_w - 2 * h_gap) / 3      # 3 columnas
        img_h    = img_w * 0.75                     # proporción 4:3

        y_grid = y - 0.6 * cm                       # margen entre texto e imágenes
    ...
    def clear_form(self):
        ...
        self.lbl_img_patient.configure(text="Paciente Actual: Ninguno Seleccionado")
        self.image_paths  = [None] * 9
        self._ctk_images  = [None] * 9
